...
public class CheckMarkArrayAdapter extends DocArrayAdapter {
    public CheckMarkArrayAdapter(@NonNull Context context, int resource, @NonNull List<BaseRec> recList) {
        super(context, resource, recList);
    }

    @Override
    protected DocRecHolder getHolderOfRow(View row, boolean needCreate) {
        return new CheckMarkRecHolder(row);
    }
}
